  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:lambda:${local.region}:${local.account_id}:function:serveml-*"
      },
      {
        Effect = "Allow"
        Action = [
...
# IAM Role for model Lambda functions
resource "aws_iam_role" "model_lambda" {
  name = "${var.project_name}-model-lambda-${var.environment}"
...
# Attach basic execution policy to model Lambda role
resource "aws_iam_role_policy_attachment" "model_lambda_basic" {
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
  role       = aws_iam_role.model_lambda.name
}

# Policy for model Lambda to access S3 for model files
resource "aws_iam_role_policy" "model_lambda_s3" {
  name = "${var.project_name}-model-lambda-s3-${var.environment}"
